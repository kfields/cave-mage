...
class KinematicCharacterController(CharacterController):
    def __init__(self, avatar: Node2D):
        ...
        #
        self.character_layer = cave_mage.globe.scene.character_layer
        self.ground_layer = cave_mage.globe.scene.ground_layer
        self.ladder_layer = cave_mage.globe.scene.ladder_layer
    ...
    def mount(self):
        ...
        for node in hit_list:
            ...
            if isinstance(node, cave_mage.characters.Skateboard):
                logger.debug(f"Mounting {node}")
                # mount = node.group
                mount = node
                mount.mount(self.avatar)
                cave_mage.globe.screen.push_avatar(mount)
