...
from cave_mage.tile import TileLayer
...
class TileLevel(Level):
    ...
    def check_butterflies(self):
        hit_list = self.butterfly_layer.query_intersection(cave_mage.globe.avatar.bounds)
        for node in hit_list:
            if cave_mage.globe.player.collect(node):
    ...
    def check_flags(self):
        hit_list = self.flag_layer.query_intersection(cave_mage.globe.avatar.bounds)
        for node in hit_list:
            if cave_mage.globe.player.collect(node):
                # Remove the flag
                node.destroy()
